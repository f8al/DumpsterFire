...
class download_hacking_tools( FireModule ):
        ...
        def Ignite( self ):
                ...
                urllib.request.urlretrieve( 'https://github.com/gentilkiwi/mimikatz/archive/master.zip', self.filepath )

                self.filepath = self.mDirectoryPath + 'responder.zip'
                print( self.commentsStr + ": Downloading Responder to: " + self.filepath )
                urllib.request.urlretrieve( 'https://github.com/SpiderLabs/Responder/archive/master.zip', self.filepath )

                self.filepath = self.mDirectoryPath + 'scapy.zip'
                print( self.commentsStr + ": Downloading Scapy to: " + self.filepath )
                urllib.request.urlretrieve( 'https://github.com/secdev/scapy/archive/v2.3.2.zip', self.filepath )

                self.filepath = self.mDirectoryPath + 'sqlmap.zip'
                print( self.commentsStr + ": Downloading SQLMap to: " + self.filepath )
                urllib.request.urlretrieve( 'https://github.com/sqlmapproject/sqlmap/archive/master.zip', self.filepath )
                ...
                urllib.request.urlretrieve( 'https://github.com/danielmiessler/SecLists/blob/master/Passwords/default-passwords.csv', self.filepath )
                ...
                urllib.request.urlretrieve( 'https://github.com/danielmiessler/SecLists/blob/master/Passwords/10k_most_common.txt', self.filepath )
